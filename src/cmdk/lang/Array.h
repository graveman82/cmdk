...
public:
	...
	{
		T itemCopy( item );
		// Зачем Kajala поставил тут >= (а не == )? Поставил ловушку и ассерт, чтобы поймать такую ситуацию.
		if ( m_len >= m_cap )
		{
			if (m_len > m_cap)
			{
	...
	void add( int index, const T& item )
	{
		...
		T itemCopy( item );
		setSize( m_len + 1 );
		rcopy( m_data+(index+1), m_data+index, m_len-(index+1) ); // сдвиг элементов с позиции index на одну позицию вправо
...
private:
	T*		m_data;
	int		m_cap; // емкость никогда не уменьшается при операциях
	int		m_len;
	T*		m_dynamicBuffer;
	T		m_staticBuffer[N];
	
	void realloc( int size ) // судя по коду - вызывается только, если нужно выделить больший объем памяти
	{
		if ( size > N )
		{
			...
			T* data = new T[cap];
			int count = m_len;
			if ( count > cap )// не понятно зачем Kajala вставил эту проверку. Поставил ловушку и ассерт, чтобы поймать такую ситуацию.
			{
			...
			{
				data[i] = m_data[i];
				m_data[i] = T(); // вобщем-то бессмысленная операция (статическим этот массив больше не станет)
			}
			delete[] m_dynamicBuffer;
			m_dynamicBuffer = 0;
		...
		else
		{
			...
			assert(m_dynamicBuffer == 0); // если буфер стал динамическим, то он им и останется 
			if ( m_len > N )// не понятно зачем Kajala вставил эту проверку. Поставил ловушку и ассерт, чтобы поймать такую ситуацию.
			{
