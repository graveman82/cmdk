...
// M.Sungatullin [25.12.2016]: в DirectX9 2010 june not Pass but is a pair BeginPass/EndPass
#ifdef _MSC_VER
	#if _MSC_VER >= 1500 // MSVC 2008 и старше
		void Dx9Effect::beginPass( int pass ) 
		{
			assert( m_fx );

			HRESULT hr = m_fx->BeginPass( pass );
			if ( hr != D3D_OK )
				error( "Dx9Effect.BeginPass() failed: %s", toString(hr) );
		}
		...
		void Dx9Effect::endPass() 
		{
			...
			if ( hr != D3D_OK )
				error( "Dx9Effect.EndPass() failed: %s", toString(hr) );
		}
	#else
